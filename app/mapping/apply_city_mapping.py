...
- Writes mapped copies to app/mapping/output by default.
...
from app.mapping.llm_utils import load_json_list as load_json_list_base

LOGGER = logging.getLogger(__name__)

DEFAULT_INPUT_DIR = Path(__file__).resolve().parent.parent / "extraction" / "output"
...
def load_json_list(path: Path) -> tuple[list[dict] | None, str]:
    ...
    Uses the centralized load_json_list_base from llm_utils but wraps
    exceptions into status tuples for backward compatibility.
    
    Status values:
    - "ok": Successfully loaded a non-empty list
    - "missing": File does not exist (returns empty list)
    - "invalid json": File exists but contains malformed JSON
    - "expected list at top-level": File contains valid JSON but not a list
    """
    try:
