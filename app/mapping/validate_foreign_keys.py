...
Run:
    python -m app.mapping.validate_foreign_keys [path-to-step3-dir]
Defaults to app/mapping/workflow_output/step3_llm when no path is provided.
"""
...
BASE_DIR = Path(__file__).resolve().parent / "workflow_output" / "step3_llm"
...
def load_json_list(path: Path) -> list[dict] | None:
    ...
    Uses the centralized load_json_list_base from llm_utils.
    Returns None for missing files, raises ValueError on corruption.
    """
    try:
