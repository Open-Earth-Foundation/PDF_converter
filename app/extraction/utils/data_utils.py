...
def parse_record_instances(
    ...
    for idx, raw in enumerate(raw_items):
        ...
        try:
            ...
            errors.append(error_msg)
            LOGGER.warning("[%s] Validation failed for item %d: %s", model_cls.__name__, idx, exc)
            continue
...
def handle_response_output(response: Response) -> tuple[list[ResponseFunctionToolCall], list[str]]:
